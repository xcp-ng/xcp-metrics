...
impl PCpuAvg {
    ...
    fn get_simple_metric(&mut self, cpuinfos: &[xc_cpuinfo_t]) -> SimpleMetric {
        let idle_time_sum: f64 = self
            .latest_idle_time
            .iter_mut()
            .zip(cpuinfos.iter())
            .filter_map(|(latest_idle_time, cpuinfo)| {
                if let Some(idle_time) = latest_idle_time {
                    // Get and replace previous cpu time.
                    let previous_idle_time = *idle_time;
                    *idle_time = cpuinfo.idletime;
                    ...
                    // We don't have the previous time.
                    latest_idle_time.replace(cpuinfo.idletime);
        ...
        SimpleMetric {
            labels: vec![],
            value: MetricValue::Gauge(NumberValue::Double(idle_time_sum / cpuinfos.len() as f64)),
        }
    }
}

impl XenMetric for PCpuAvg {
    fn get_family(&mut self, shared: &XenMetricsShared) -> Option<(Box<str>, SimpleMetricFamily)> {
        self.latest_idle_time.resize(shared.cpuinfos.len(), None);

        let metrics = vec![self.get_simple_metric(&shared.cpuinfos)];
        ...
        Some((
            ...
            SimpleMetricFamily {
                metric_type: MetricType::Gauge,
                unit: "".into(),
                help: "Average physical cpu usage".into(),
                metrics,
            },
        ))
    }
